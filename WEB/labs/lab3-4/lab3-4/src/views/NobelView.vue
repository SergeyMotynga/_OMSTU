<template>
  <div>
    ...
    <div class="filters">
      <input v-model="searchQuery" placeholder="Поиск по имени" @input="onFilterChange" />
...
import type { Laureate } from '@/services/features/nobel/codecs/LaureateCodec'
import type { LaureatesResponse } from '@/services/features/nobel/codecs/ApiResponseCodecs'
...
const data = ref<Laureate[]>([])
const totalItems = ref(0)
const hasMorePages = ref(false)

const columns = ['Имя', 'Дата рождения', 'Число премий', 'Категория', 'Год']
...
const load = async () => {
  ...
  try {
    ...
    const res: LaureatesResponse = await nobelService.fetchLaureates(params)
    data.value = res.laureates
    totalItems.value = res.laureates.length
    hasMorePages.value = res.laureates.length === limit.value
  } catch (e) {
    console.error(e)
    data.value = []
    totalItems.value = 0
...
const rowsToShow = computed(() => {
  ...
  if (searchQuery.value) {
    ...
    filtered = filtered.filter(l => {
      const name = (l.knownName?.en ?? l.fullName?.en ?? '').toLowerCase()
      return name.includes(query)
    })
  }

  return filtered.map(l => ({
    'Имя': l.knownName?.en ?? l.fullName?.en ?? '–',
    'Дата рождения': l.birth?.date ?? '–',
    'Число премий': l.nobelPrizes?.length ?? 0,
    'Категория': '–',
    'Год': l.nobelPrizes?.[0]?.awardYear ?? '–',
  }))
})
